...
use crate::{
    ...
    r#gen::{
        CancelFlightInfoRequest, CancelFlightInfoResult, FlightInfoView, HandshakeResponseView,
...
use connectrpc::{
    ServerStream,
    client::{ClientConfig, SharedHttp2Connection},
};
use connectrpc::{ServiceResult, ServiceStream, http_body::Body};
use futures::{
...
/// # }
/// ```
pub struct FlightClient<T = SharedHttp2Connection> {
...
where
    ...
    T::ResponseBody: Body<Data = Bytes> + std::marker::Send + 'static + Unpin,
    <T::ResponseBody as Body>::Error: ::std::fmt::Display,
{
    /// Creates a client with the provided transport
    pub fn new(inner: T, config: ClientConfig) -> Self {
        Self::new_from_inner(FlightServiceClient::new(inner, config))
    }
    ...
    pub fn add_header(&mut self, key: &str, value: &str) -> Result<()> {
        let key = HeaderName::from_str(key).map_err(|e| FlightError::ExternalError(Box::new(e)))?;
    ...
    pub async fn handshake(&mut self, payload: impl Into<Bytes>) -> Result<Bytes> {
        let mut bidi_stream = self.inner.handshake().await?;
        ...
        if let Some(response) = bidi_stream.message().await? {
            // check if there is another response
            if bidi_stream.message().await?.is_some() {
                return Err(FlightError::protocol(
                    "Got unexpected second response from handshake",
                ));
            }
            Ok(response.to_owned_message().payload)
        } else {
    ...
    pub async fn do_get(&mut self, ticket: Ticket) -> Result<FlightRecordBatchStream> {
        let server_stream = self.inner().do_get(ticket).await?;

        // let (md, response_stream, _ext) = self.inner.do_get(ticket).await?.into_parts();
        // let (response_stream, trailers) = extract_lazy_trailers(response_stream);

        Ok(
            FlightRecordBatchStream::new_from_flight_data(server_stream), // .with_headers(md)
    ...
    pub async fn get_flight_info(&mut self, descriptor: FlightDescriptor) -> Result<FlightInfo> {
        let response = self.inner.get_flight_info(descriptor).await?.into_owned();
        Ok(response)
    }
    ...
    pub async fn poll_flight_info(&mut self, descriptor: FlightDescriptor) -> Result<PollInfo> {
        let response = self.inner.poll_flight_info(descriptor).await?.into_owned();
        Ok(response)
    }
    ...
    pub async fn do_put<S: Stream<Item = Result<FlightData>> + Send + 'static>(
        ...
        let response_stream = self.inner.do_put().await?;
    ...
    pub async fn do_exchange<S: Stream<Item = Result<FlightData>> + Send + 'static>(
        ...
        let response_stream = self.inner.do_exchange().await?.into_inner();
    ...
    pub async fn list_flights(
        ...
        let stream = self.inner.list_flights(request).await?;

        let out = futures::stream::try_unfold(stream, |mut stream| async move {
            match stream.message().await {
                Ok(Some(msg)) => {
                    let owned = msg.to_owned_message();
                    Ok(Some((owned, stream)))
                }
                Ok(None) => Ok(None),
                Err(err) => Err(FlightError::from(err)),
            }
        });
    ...
    pub async fn get_schema(&mut self, flight_descriptor: FlightDescriptor) -> Result<Schema> {
        let schema_result = self.inner.get_schema(flight_descriptor).await?.into_owned();
    ...
    pub async fn list_actions(&mut self) -> Result<BoxStream<'static, Result<ActionType>>> {
        let stream = self.inner.list_actions(Empty {}).await?;
    ...
    pub async fn do_action(&mut self, action: Action) -> Result<BoxStream<'static, Result<Bytes>>> {
        let stream = self.inner().do_action(action).await?;

        let out = futures::stream::try_unfold(stream, |mut stream| async move {
            match stream.message().await {
                Ok(Some(msg)) => {
                    let owned = msg.to_owned_message().body;
                    Ok(Some((owned, stream)))
                }
                Ok(None) => Ok(None),
                Err(err) => Err(FlightError::from(err)),
    ...
    pub async fn cancel_flight_info(
        ...
        CancelFlightInfoResult::decode(&mut response)
    ...
    pub async fn renew_flight_endpoint(
        ...
        FlightEndpoint::decode(&mut response).map_err(|e| FlightError::DecodeError(e.to_string()))
    }

    /// return a Request, adding any configured metadata
    fn make_request<R>(&self, t: R) -> Request<R> {
        // Pass along metadata
        let mut request = Request::new(t);
        *request.headers_mut() = self.metadata.clone();
        request
    }
}
