...
pub enum FlightError {
    ...
    /// Error from the underlying connectrpc library
...
impl std::fmt::Display for FlightError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FlightError::Connect(source) => write!(f, "Connect error: {source}"),
...
impl Error for FlightError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            FlightError::Connect(source) => Some(source),
...
impl From<ConnectError> for FlightError {
    fn from(status: ConnectError) -> Self {
        Self::Connect(Box::new(status))
    }
}

impl From<buffa::DecodeError> for FlightError {
...
impl From<FlightError> for ConnectError {
    fn from(value: FlightError) -> Self {
        match value {
            FlightError::Arrow(e) => ConnectError::internal(e.to_string()),
            FlightError::NotYetImplemented(e) => ConnectError::unimplemented(e),
            FlightError::Connect(status) => *status,
            FlightError::ProtocolError(e) => ConnectError::internal(e),
            FlightError::DecodeError(e) => ConnectError::internal(e),
            FlightError::ExternalError(e) => ConnectError::internal(e.to_string()),
        }
    }
}
